namespace DatabaseClient {
    ...
    function search(_event: Event): void {
        let matrikelnummer: HTMLInputElement = <HTMLInputElement>document.getElementById("matrikelSearch");
    ...
    function handleSearch(_event: ProgressEvent): void {
        let xhr: XMLHttpRequest = (<XMLHttpRequest>_event.target);
        if (xhr.readyState == XMLHttpRequest.DONE) {
            let output: HTMLTextAreaElement = <HTMLTextAreaElement>document.getElementById("singleStudent");
            output.value = xhr.response;
            let responseAsJson: JSON = JSON.parse(xhr.response);
            console.log(responseAsJson);
        }
